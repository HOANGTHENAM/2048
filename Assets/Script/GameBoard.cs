...
public class GameBoard : MonoBehaviour
{
    ...
    public void CreateTile()
    {
        ...
        tile.SetProperties(tileProperties[0], 2);
    ...
    public void Merge(Tile a, Tile b)
    {
        ...
        int index = Mathf.Clamp(IndexOf(b.tilePros) + 1, 0, tileProperties.Length - 1); // gioi han viec tang so
        int value = b.value * 2;

        b.SetProperties(tileProperties[index], value);

        gameManager.IncreaseScore(value);
    }

    public int IndexOf(TileProperties p)
    {
        for (int i = 0; i < tileProperties.Length; i++)
        {
            if (p == tileProperties[i])
            {
                return i;
            }
        }
